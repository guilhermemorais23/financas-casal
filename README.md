...
- Auth via Firebase (Google, Apple + e-mail/senha; popup, com redirecionamento quando o popup não abre, como no app instalado no iPhone), perfil criado automaticamente no primeiro login (`POST /api/me/bootstrap`). **Excluir conta** (`DELETE /api/me`): apaga login, perfil, conta pessoal e tudo nela, cartões/dívidas pessoais, links e conexões; o último membro leva o grupo inteiro junto. Política de privacidade pública em `/privacidade` (contato opcional via `VITE_PRIVACY_CONTACT_EMAIL`).
...
- Cartões (`/api/cards`): compras, fatura atual, lembrete de vencimento, limite disponível agora e quanto volta a cada fatura paga. Cartão com **limite garantido** (o dinheiro guardado vira o limite): guardar mais e resgatar (`POST /api/cards/:id/secured-limit`), resgate só até o disponível. Guardar sai da conta como **transferência** (`securedCardId` no lançamento): mexe no saldo, mas fica fora de Entrada/Saída, relatórios e orçamento; a fatura continua sendo paga pela conta.
...
- Frontend completo: Painel (individual, com widgets e cache instantâneo entre meses já visitados; "Seu dinheiro hoje" = contas + guardado em cartões, quanto dá pra gastar por dia até o fim do mês, aviso quando a maior parte dos gastos está sem categoria), Par (conjunto, com orçamento e saldo por pessoa), Nova transação, Metas, Cartões, Dívidas, Lista de compras, Investimentos, Relatórios (gráficos, busca, visão anual, exportar), Conta (membros, contas, categorias, convite, desvincular). PWA com service worker (funciona offline pro shell, dados sempre vêm da rede).
- Agilidade: menu lateral fixo na tela; excluir lançamento/meta some na hora com animação e tem **Desfazer** (o DELETE só vai ao servidor depois de ~6 s); nova despesa/receita e aporte em meta aparecem antes do servidor responder (falhou: volta atrás e oferece "Tentar de novo"). Confirmações usam o diálogo do app (`useConfirm`), nunca `window.confirm`.
- Exportar (Relatórios): PDF do mês (tela de impressão do navegador), CSV, link de leitura do extrato (`/api/shares`, público em `/r/:token`, expira em 7 dias, revogável, nunca inclui lançamentos privados nem a conta pessoal do parceiro).
- Importar extrato do banco (`/api/statements/preview` e `/commit`): OFX e CSV, com tela de revisão, duplicados desmarcados e categoria sugerida pelo histórico. PDF e Open Finance ("Conectar conta") ainda não: o botão existe como "Em breve".
